...
void LEDBlinker::blink(unsigned int const blinkOnDuration,
  ...
  mBlinkOnDuration = blinkOnDuration ? _max(LED_BLINKER_MIN_INTERVAL, blinkOnDuration) : 0;
  mBlinkOffDuration = blinkOffDuration ? _max(LED_BLINKER_MIN_INTERVAL, blinkOffDuration) : 0;
  mNumBlinks = numBlinks;
  mPauseDuration = pauseDuration ? _max(LED_BLINKER_MIN_INTERVAL, pauseDuration) : 0;;
